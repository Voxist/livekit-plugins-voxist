...
class VoxistSTTStream(RecognizeStream):
    ...
    async def _run(self) -> None:
        ...
        while reconnect_attempts <= max_attempts:
            try:
                ...
                logger.debug(f"Stream {self._session_id} completed normally")
                break
        ...
        logger.debug(f"Stream {self._session_id} finished")
        ...
        try:
            ...
            while not self._conn.ws.closed:
                ...
                if msg.type == aiohttp.WSMsgType.TEXT:
                    # Parse JSON message
                    logger.debug(f"Stream {self._session_id} received from Voxist: {msg.data[:200]}")
                    try:
    ...
    async def _process_result(self, data: dict):
        ...
        # Final results (confirmed transcription)
            ...
            if text:
                logger.debug(f"Stream {self._session_id} final: {text[:100]}")
